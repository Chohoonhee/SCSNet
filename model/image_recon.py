...
class MSResNet_with_event(nn.Module):
    def __init__(self, args):
        ...
        # ])
        self.body_models = nn.ModuleList([
            ResNet_event(args, 5, 3, mean_shift=False),
        ])
        # for _ in range(1, self.n_scales):
        #     self.body_models.insert(0, ResNet_event(args, 11, 3, mean_shift=False))
        for _ in range(1, self.n_scales):
            self.body_models.insert(0, ResNet_event(args, 8, 3, mean_shift=False))
    ...
    def forward(self, input_pyramid):
        ...
        input_pyramid = [None] * self.n_scales
        output_pyramid = [None] * self.n_scales

        
        # for s in scales:
        #     input_pyramid[s] = torch.cat((image_pyramid[s], event_pyramid[s]), 1)

        for s in scales:
            input_pyramid[s] = event_pyramid[s]
