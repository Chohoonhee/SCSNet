...
class Decoder_fusion(nn.Module):
    def __init__(self, embed_dim=384, token_dim=64, depth=2, img_size=224):
        ...
        self.img_size = img_size
        # token upsampling and multi-level token fusion
        self.decoder1 = fusion_module(dim=embed_dim, token_dim=token_dim, img_size=img_size, ratio=8, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), fuse=True)
    ...
    def forward(self, image_fea_1_16, event_fea_1_16):

        B, _, _, = image_fea_1_16.size()

        image_fea_1_16 = self.mlp(self.norm(image_fea_1_16))
        # saliency_fea_1_16 [B, 14*14, 64]

        event_fea_1_16 = self.mlp_c(self.norm_c(event_fea_1_16))
        # contour_fea_1_16 [B, 14*14, 64]

        # reverse T2T and fuse low-level feature
        fused_feature_1_16 = self.decoder1(image_fea_1_16, event_fea_1_16, self.img_size)

 
        return fused_feature_1_16
...
class fusion_module(nn.Module):
    ...
    def forward(self, dec_fea, enc_fea=None, image_size=8):
        ...
        dec_fea = dec_fea.transpose(1, 2).reshape(B, C, 320//image_size, 384//image_size)
...
class Decoder_my(nn.Module):
    def __init__(self, embed_dim=384, token_dim=64, depth=2, img_size=224):
        ...
        super(Decoder_my, self).__init__()
    ...
    def forward(self, saliency_fea_1_16, token_fea_1_16, saliency_tokens, contour_fea_1_16, contour_tokens, rgb_fea_1_8, rgb_fea_1_4):
        ...
        # saliency_fea_1_16 [B, 14*14, 64]
        mask_1_16 = self.pre_1_16(saliency_fea_1_16)
        mask_1_16 = mask_1_16.transpose(1, 2).reshape(B, 1, self.height // 16, self.width // 16)

        contour_fea_1_16 = self.mlp_c(self.norm_c(contour_fea_1_16))
        # contour_fea_1_16 [B, 14*14, 64]
        contour_1_16 = self.pre_1_16_c(contour_fea_1_16)
        contour_1_16 = contour_1_16.transpose(1, 2).reshape(B, 1, self.height // 16, self.width // 16)
        ...
        # predict saliency and contour maps
        mask_1_8 = self.pre_1_8(saliency_fea_1_8)
        mask_1_8 = mask_1_8.transpose(1, 2).reshape(B, 1, self.height // 8, self.width // 8)

        contour_1_8 = self.pre_1_8_c(contour_fea_1_8)
        contour_1_8 = contour_1_8.transpose(1, 2).reshape(B, 1, self.height // 8, self.width // 8)

        # 1/8 -> 1/4
        fea_1_4 = self.decoder2(token_fea_1_8[:, 1:-1, :], rgb_fea_1_4).transpose(1, 2).reshape(B, -1, self.height // 4, self.width // 4)
        ...
        # # predict saliency maps and contour maps
        # mask_1_4 = self.pre_1_4(saliency_fea_1_4)
        # mask_1_4 = mask_1_4.transpose(1, 2).reshape(B, 1, self.height // 4, self.width // 4)

        # contour_1_4 = self.pre_1_4_c(contour_fea_1_4)
        # contour_1_4 = contour_1_4.transpose(1, 2).reshape(B, 1, self.height // 4, self.width // 4)
