...
def show_multi_image(overlay, image, event):
    cv2.namedWindow('viz', cv2.WND_PROP_FULLSCREEN)
    # import pdb; pdb.set_trace()
    image = np.repeat(image[..., np.newaxis], 3, axis=2)
    image = np.hstack((overlay, image))
    image = np.hstack((image, event))
    cv2.imshow('viz', image)
    cv2.waitKey(0)
...
def show_multi_overlay(image_1c, disp_rgb_image, height, width):
    overlay = get_disp_overlay(image_1c, disp_rgb_image, height, width)
    show_multi_image(overlay,image_1c, disp_rgb_image)
