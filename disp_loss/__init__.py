...
from .metric import One_PA, MAE, Three_PA, Two_PA, RMSE, Mean_Depth, Mean_Disp
from disp_loss import disp_loss as dl
...
class Loss(torch.nn.modules.loss._Loss):
    def __init__(self, args, epoch=None, model=None, optimizer=None):
        ...
        self.save_name = os.path.join(self.save_dir, 'disp_loss.pt')
        ...
        for weighted_loss in args.loss.split('+'):
            ...
            # if args.model in ['psm', 'deblur_psm', 'HINet_stereo', 'HINet_mono']:
            #     loss_type = 'L1'
            #     func = dl.smoothL1Loss()
            # else:
            #     loss_type = 'MULTI_SCALE'
            #     func = dl.multiscaleLoss(args.maxdisp)

            if args.disp_model in ['pasm_pertu_noise']:
                loss_type = 'PASM'
                func = dl.pasmLoss()
            else:
                loss_type = 'L1'
        ...
        print('Loss function: {}'.format(self.loss_types[0]))

        # Metrics
        self.do_measure = args.metric.lower() != 'none'

        self.metric = nn.ModuleDict()
        # self.metric_types = ['1PA', '2PA', '3PA', 'RMSE']
        self.metric_types = ['1PA', 'Mean_Depth', 'Mean_Disp']
        ...
        if self.do_measure: 
            for metric_type in self.metric_types:
                ...
                if metric_type == '1PA':
                    metric_func = One_PA()
                elif metric_type == '2PA':
                    metric_func = Two_PA()
                elif metric_type == '3PA':
                    metric_func = Three_PA()
                elif metric_type == 'Mean_Depth':
                    metric_func = Mean_Depth()
                elif metric_type == 'Mean_Disp':
                    metric_func = Mean_Disp()
                elif metric_type == 'RMSE':
                    metric_func = RMSE()    # single precision
                else:
                    raise NotImplementedError

                self.metric[metric_type] = metric_func

        print('Metrics: {}'.format(self.metric_types))
    ...
    def forward(self, input, target):
        ...
        for loss_type in self.loss_types:
            ...
            if loss_type == 'ADV':
                ...
                _loss = self.loss[loss_type](input, target) * self.weight[loss_type]
            elif loss_type == 'L1':
                
                _loss = _ms_forward(input, target, self.loss[loss_type])    
            else:
    ...
    def measure(self, input, target):
        ...
        for metric_type in self.metric_stat[self.mode]:
            ...
            _metric = self.metric[metric_type](input.unsqueeze(1), target, 37)
            
            
            # self.metric_stat[self.mode][metric_type][self.epoch] += _metric.item() * count
            
            if metric_type == 'RMSE':
                self.metric_stat[self.mode][metric_type][self.epoch] += _metric * count
            else:
                self.metric_stat[self.mode][metric_type][self.epoch] += _metric * count
    ...
    def plot_loss(self, plot_name=None):
        if plot_name is None:
            plot_name = os.path.join(self.save_dir, "{}_disp_loss.pdf".format(self.mode))
    ...
    def plot_metric(self, plot_name=None):
        ...
        if plot_name is None:
            plot_name = os.path.join(self.save_dir, "{}_disp_metric.pdf".format(self.mode))
        ...
        for metric_type, metric_record in self.metric_stat[self.mode].items():
            ...
            # value = [metric_record[epoch].cpu() if metric_record[epoch].is_cuda() else metric_record[epoch] for epoch in axis]
